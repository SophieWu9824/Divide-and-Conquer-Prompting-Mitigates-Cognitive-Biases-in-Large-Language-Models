% % original plot
function barplot_bias1(biasData,model_names,bias_names,id)
% id = [5,6,2,7,1,3,4];
% 7 biases * 7 models
frame_dat = biasData.FramingEffect;
anchor_dat = biasData.AnchorEffect;
confirm_dat = biasData.ConfirmationBias;
represent_dat = biasData.RepresentativeHeuristic;
conjunc_dat = biasData.ConjunctionFallacy;
endow_dat = biasData.EndowmentEffect;
insen_dat = biasData.InsensitivityToSampleSize;
...
numBars = 2; % Phone_A, Phone_B
groupWidth = min(0.5,numBars/(numBars+1));
means = table2array(represent_dat(:,[3,4]));
for bar_i = 1:numBars
    x = (1:numGroups)-groupWidth/2+(2*bar_i-1)*groupWidth/(2*numBars);
    bar(x,means(:,bar_i)',groupWidth/numBars,'DisplayName',model_names(bar_i),...
        'FaceColor',colors{bar_i});
...
numBars = 2; % old,new
groupWidth = min(0.5,numBars/(numBars+1));
means = table2array(endow_dat(:,[3,4]));
temp_colors = {'b','r','y'};
for bar_i = 1:numBars
    x = (1:numGroups)-groupWidth/2+(2*bar_i-1)*groupWidth/(2*numBars);
    bar(x,means(:,bar_i)',groupWidth/numBars,'DisplayName',model_names(bar_i),...
...
numBars = 2; % bigger,smaller
groupWidth = min(0.5,numBars/(numBars+1));
means = table2array(insen_dat(:,[4,5]));
for bar_i = 1:numBars
    x = (1:numGroups)-groupWidth/2+(2*bar_i-1)*groupWidth/(2*numBars);
    bar(x,means(:,bar_i)',groupWidth/numBars,'DisplayName',model_names(bar_i),...
        'FaceColor',colors{bar_i});
...
ylabel("Proportion")
yline(0.5,'--','LineWidth',0.5,'Color','k')
legend("bigger hospital","smaller hospital")
title(bias_names(7))
